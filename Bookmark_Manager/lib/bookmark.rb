...
class Bookmark
  ...
  def self.where(tag_id:)
    result = DatabaseConnection.query("SELECT bookmarks.id, url, title FROM bookmark_tags INNER JOIN bookmarks ON bookmarks.id = bookmark_tags.bookmark_id WHERE bookmark_tags.tag_id = '#{tag_id}';")
